    ...
    <Box sx={styles.wrapper}>
      ...
      <Box sx={styles.contentWrapper}>
        <Content
          title="운영체제 및 실험"
          subTitle="핀토스 프로젝트"
          description="방학때부터 미리 프로젝트를 준비하여, 작은 os라는 뜻의 pintOS 프로젝트를 2인에서 진행. 처음으로 큰 규모의 코드를 접하는 경험. Thread 스캐쥴링부터 시작해서, stack 관리를 통해 function call, system call 등을 관리했고, 나아가 파일 시스템을 구현하였다. 이를 통해 운영체제의 기본적인 개념을 이해하고, 실제로 구현해보는 경험을 했다."
        />
        <Content
          title="파이썬을 통한 자연언어처리"
          subTitle="영화 대본에서 등장인물 별 성격 클러스터링"
          description="여러 영화의 대본을 통해 등장인물의 대사를 수집하여, 각 등장인물의 대사를 통해 성격을 분석하고, 클러스터링을 통해 성격을 분류하였다. 같은 배우가 연기한 등장인물들이 비슷한 성격을 가지는지에 대한 가설을 세우고, 이를 통해 성격을 분류하였다. 하지만 클러스터링 한 결과, 배우와 관계 없이 같은 영화에 있는 등장인물들이 같은 클러스터에 모였다. 영화별로 고유의 분위기가 있어서, 이를 통해 성격을 분류하는 것은 어려움을 알게 되었다."
        />
        <Content
          title="컴퓨터 그래픽스 개론"
          subTitle="OpenGL 기반 기본적인 그래픽스 개념"
          description="OpenGL을 통해 3D 그래픽스를 구현하였다. 수업을 통해 3D 그래픽스의 기본적인 개념을 이해하고, 주어진 skeleton 코드를 통해서 3D cow를 키보드 마우스와 연결해서 확대하고 회전시키는 등의 구현을 했고, 빛과 카메라의 위치를 통해 빛 반사와 그림자를 최대한 자연스럽게 구현했다. 이를 통해 3D 그래픽스의 기본적인 개념을 이해하게 되었다."
        />
      </Box>
      <Box sx={styles.contentWrapper}>
        직전 학기에 들은 AI 관련 과목들
      ...
      <Box sx={styles.contentWrapper}>
        ...
        <Content
          title="프론트엔드"
          subTitle="React, Material-UI, nextJS, TypeScript로 기획 구현 및 native 개발자와 협업"
